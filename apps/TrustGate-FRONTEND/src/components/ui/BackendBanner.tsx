...
import { Server, CheckCircle2, AlertTriangle, Settings2, RefreshCw, X } from 'lucide-react';
...
export function BackendBanner({ onBackendUrlChanged }: BackendBannerProps) {
  ...
  const handleTestConnection = async () => {
    ...
    try {
      ...
      const res = await fetch(`${currentUrl}/api/merchant/all/latest-verification`, {
        method: 'GET',
  ...
  return (
    <div className="bg-[#18181B] text-zinc-300 text-xs px-4 py-2 border-b border-[#27272A] flex flex-wrap items-center justify-between gap-3 font-mono">
      <div className="flex items-center gap-2">
        ...
        <span className="text-[#A1A1AA] font-medium text-[11px] uppercase tracking-wider">Backend API:</span>
        ...
        {testStatus === 'error' && (
          <span className="inline-flex items-center gap-1 text-red-400 font-medium ml-1" title={errorMessage}>
      ...
      {isEditing && (
        <div className="w-full mt-2 pt-2 border-t border-[#27272A] flex flex-wrap items-center gap-2 font-sans">
          <label htmlFor="backend-url-input" className="text-zinc-400 text-xs">Target Backend URL:</label>
          <input
