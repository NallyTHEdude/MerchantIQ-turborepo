  ...
  const loadMerchantDetails = useCallback(async () => {
    ...
    try {
      ...
      const matched = allMerchants.find(
        (m) => m.merchant?.id === merchantId || m.verification?.merchantId === merchantId
      );
      ...
      setError(err instanceof Error ? err.message : 'Failed to retrieve merchant profile.');
    } finally {
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pt-2">
        ...
        <div className="flex items-center gap-2.5">
          <button
            ...
            disabled={isLoadingMerchant || isLoadingVerifications || isLoadingPayments}
          ...
          >
            <RefreshCw
              className={`w-3.5 h-3.5 text-[#71717A] ${
                isLoadingMerchant || isLoadingVerifications || isLoadingPayments
                  ? 'animate-spin'
                  : ''
              }`}
            />
      ...
      <div className="bg-white rounded-xl border border-[#E4E4E7] p-6 shadow-xs">
        {isLoadingMerchant ? (
          ...
          <div className="p-4 bg-red-50 border border-red-200 rounded-lg text-red-800 flex items-center gap-3 text-sm">
            ...
            <div>
              <p className="font-semibold">Unable to fetch complete merchant info</p>
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-5 border-b border-[#E4E4E7]">
              ...
              {merchant.createdAt && (
                <div className="text-xs text-[#71717A] font-mono flex items-center gap-1.5">
                  ...
                  <span>Enrolled: {formatDate(merchant.createdAt)}</span>
                </div>
              )}
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 pt-5">
              ...
              <div>
                ...
                {merchant.websiteUrl ? (
                  ...
                  <span className="text-sm text-[#A1A1AA] font-mono">Not provided</span>
                )}
              </div>
      ...
      <div className="space-y-4 pt-2">
        <div>
          ...
          <p className="text-xs text-slate-500">
            Real-time aggregate transaction metrics and settlement channels
          </p>
        </div>
