...
function MerchantRow({
    ...
    return (
        <TableRow
            className="group cursor-pointer border-border/70 hover:bg-secondary/50"
        ...
        >
            ...
            <TableCell className="border-r border-border/40 whitespace-nowrap text-xs text-muted-foreground">
                {merchant.category}
            </TableCell>
...
export function MerchantQueue({
    ...
    const [category, setCategory] = useState<
        'all' | (typeof merchantCategories)[number]
    >('all');
    const [sortKey, setSortKey] = useState<
        'merchant' | 'status' | 'risk' | 'trustScore' | 'category' | 'updated'
    >('updated');
        ...
        () =>
            records
                ...
                .sort((a, b) => {
                    ...
                    > = {
                        merchant: [a.name, b.name],
                        status: [a.status, b.status],
                        risk: [a.risk, b.risk],
                        trustScore: [a.trustScore, b.trustScore],
        ...
        <section className="flex flex-col gap-3">
            <div className="flex flex-col gap-1">
                ...
                <p className="text-xs text-muted-foreground">
                    Review the latest merchant verification submissions and
                    exceptions.
                </p>
            </div>
                ...
                <div className="flex flex-wrap items-center gap-2 lg:ml-auto">
                        ...
                        <Select
                            ...
                            onValueChange={(value) =>
                                setCategory(
                                    value as
                                        | 'all'
                                        | (typeof merchantCategories)[number],
                                )
                            }
                        >
                            <SelectTrigger
                    ...
                    <div className="flex items-center gap-1.5">
                        ...
                        <Select
                            value={risk}
                            onValueChange={(value) =>
                                setRisk(value as 'all' | RiskLevel)
                            }
                        >
                            <SelectTrigger
                                id="risk-filter"
                            ...
                            <TableHeader className="bg-card/60">
                                <TableRow className="border-border bg-card/60 hover:bg-card/60">
                                    {(
                                        ...
                                        <TableHead
                                            key={key}
                                            className={`h-10 border-r border-border/40 text-[11px] font-medium text-muted-foreground ${index === 0 ? 'px-4' : ''}`}
                                        >
                                            <button
                                                type="button"
                                                onClick={() => toggleSort(key)}
                                                className="inline-flex items-center gap-1.5 rounded px-1 py-1 transition-colors hover:bg-secondary hover:text-foreground"
                                                aria-label={`Sort by ${label} ${sortKey === key && sortAsc ? 'descending' : 'ascending'}`}
                                            >
                                                <span>{label}</span>
                                                <ArrowUpDown
                                                    className={`size-3 ${sortKey === key ? 'text-primary' : 'text-muted-foreground/60'}`}
                                                    aria-hidden="true"
                                                />
                                                {sortKey === key && (
                                    ...
                                    <TableHead className="h-10 w-8 border-l border-border/40" />
                                </TableRow>
                            </TableHeader>
                        </Table>
                    </div>
