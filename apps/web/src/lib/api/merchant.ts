...
export async function createMerchant(
    ...
    const response = await request<
        Merchant | { id?: string; merchant?: Merchant }
    >('/api/merchant', {
        method: 'POST',
    ...
    if (rawData && typeof rawData === 'object') {
        if (
            'id' in rawData &&
            typeof (rawData as { id: string }).id === 'string'
        ) {
    ...
    if (
        !merchantId &&
        'id' in response &&
        typeof (response as unknown as { id: string }).id === 'string'
    ) {
...
export async function deleteMerchant(
    ...
    return request<Merchant>(
        `/api/merchant/${encodeURIComponent(merchantId)}`,
        {
            method: 'DELETE',
        },
    );
