...
export function RiskOverview({ latestVerification, isLoading }: RiskOverviewProps) {
  if (isLoading) {
    return (
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 pt-5">
          ...
          <div className="bg-[#FAFAFA] rounded-lg p-4 border border-[#E4E4E7]">
            <div className="flex items-center justify-between">
              ...
              <span className="text-[10px] font-mono text-[#71717A]">0 - 100</span>
            </div>
            <div className="mt-2 flex items-baseline gap-2">
              <span className="text-2xl font-bold text-[#18181B] tracking-tight font-mono">
                {trustscore}
              </span>
              <span className="text-xs font-semibold text-[#71717A]">/ 100</span>
            </div>
            {/* Visual Bar */}
            <div className="w-full bg-[#E4E4E7] rounded-full h-1.5 mt-2.5 overflow-hidden">
              <div
                ...
                style={{ width: `${Math.min(Math.max(trustscore, 5), 100)}%` }}
              />
            </div>
          </div>
