...
export function TrustScoreChart({
  ...
  return (
    <div id="chart-trust-score-container" className="bg-white rounded-xl border border-[#E4E4E7] shadow-xs overflow-hidden">
      <div className="p-4 sm:p-5 border-b border-[#E4E4E7] flex flex-row items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <h4 className="text-sm font-semibold text-[#18181B] tracking-tight">Trust Score Over Time</h4>
        </div>
        ...
        {isLoading ? (
          <div className="h-64 flex items-center justify-center">
            <div className="animate-pulse flex flex-col items-center gap-2">
              ...
              <span className="text-xs text-[#71717A]">Loading trust telemetry...</span>
            </div>
          </div>
        ) : chartData.length === 0 ? (
          ...
          <div>
                      ...
                      if (active && payload && payload.length) {
                        ...
                        return (
                          <div className="bg-[#18181B] text-white text-xs p-3 rounded-lg shadow-lg border border-[#27272A] space-y-1">
                            <p className="text-[#A1A1AA] font-mono text-[10px]">{data.date}</p>
                            <p className="font-semibold text-xs flex items-center gap-1.5 font-mono">
                              Score: <span className="text-emerald-400 font-bold">{data.trustscore}</span> / 100
                            </p>
                            <p className="text-[#D4D4D8] text-[11px]">
                              Risk: <span className="font-semibold text-white">{data.riskLevel}</span>
                            </p>
                            <p className="text-[#D4D4D8] text-[11px]">
                              Status: <span className="font-semibold text-white">{data.status}</span>
                            </p>
                          </div>
                        );
                      }
            ...
            {chartData.length === 1 && (
              <p className="text-center text-xs text-[#71717A] mt-2 font-mono">
                Single historical checkpoint recorded on {chartData[0].date}.
              </p>
            )}
          </div>
        )}
