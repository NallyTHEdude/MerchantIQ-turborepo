...
type SortField =
    'name' | 'category' | 'status' | 'trustscore' | 'risk' | 'createdAt';
    ...
    const filteredMerchants = useMemo(() => {
        ...
        if (statusFilter !== 'ALL') {
            result = result.filter((item) => {
                const s = (
                    item.verification?.verificationStatus || 'PENDING'
                ).toUpperCase();
        ...
        if (riskFilter !== 'ALL') {
            result = result.filter((item) => {
                const r = (item.verification?.riskLevel || '')
                    .toUpperCase()
                    .replace(/\s+/g, '_');
                    ...
                    <table className="w-full text-left border-collapse">
                        ...
                        <tbody className="divide-y divide-[#F4F4F5] text-sm">
                            {filteredMerchants.map((item) => {
                                ...
                                const merchantId =
                                    merchant?.id || verification?.merchantId;
            ...
            {!isLoading && !error && (
                <div className="px-6 py-3 border-t border-[#E4E4E7] bg-[#FAFAFA] flex items-center justify-between">
                    <div className="text-xs text-[#71717A]">
                        Showing{' '}
                        <strong className="text-[#18181B] font-semibold">
                    ...
                    <div className="text-xs text-[#71717A] hidden sm:block">
                        Click any row to inspect verification history &amp; risk
                        telemetry
                    </div>
                </div>
            )}
