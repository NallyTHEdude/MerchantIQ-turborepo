  ...
  const filteredPayments = useMemo(() => {
    ...
    if (filterMethod !== 'ALL') {
      result = result.filter(
        (p) => (p.paymentMethod || '').toUpperCase() === filterMethod.toUpperCase()
      );
    }
    ...
    result.sort((a, b) => {
      if (sortField === 'amount') {
        const amtA = typeof a.amount === 'string' ? parseFloat(a.amount) : a.amount;
        const amtB = typeof b.amount === 'string' ? parseFloat(b.amount) : b.amount;
    ...
    <div className="bg-white rounded-xl border border-[#E4E4E7] shadow-xs overflow-hidden">
      ...
      {isLoading ? (
        ...
        <div className="py-12 px-6 text-center text-[#71717A]">
          ...
          <p className="text-sm font-medium text-[#18181B]">No payment transactions found</p>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-[#FAFAFA] border-b border-[#E4E4E7] text-[#71717A] text-[10px] font-bold uppercase tracking-widest">
                ...
                <th scope="col" className="py-3 px-6 text-right">Date &amp; Time</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-[#F4F4F5] text-sm">
              {filteredPayments.map((p, index) => {
                ...
                return (
                  <tr key={p.id || index} className="hover:bg-[#F9FAFB] transition-colors">
                    ...
                    <td className="py-3.5 px-4">
                      {isIntl ? (
                        ...
                        <span className="text-xs text-[#71717A] font-mono">Domestic</span>
                      )}
                    </td>
                    <td className="py-3.5 px-6 text-right text-xs text-[#71717A] font-mono">
